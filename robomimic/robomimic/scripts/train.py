...
def train(config, device, resume=False):
    ...
    for epoch in range(start_epoch, config.train.num_epochs + 1):
        ...
        # save latest model for resume functionality (only when a ckpt is also being saved,
        # or at the very last epoch, to avoid slow I/O every single epoch)
        is_last_epoch = (epoch == config.train.num_epochs)
        should_save_latest = should_save_ckpt or is_last_epoch or (epoch % 10 == 0)
