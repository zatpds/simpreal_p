...
class OTDiffusionPolicyUNet(PolicyAlgo):
    ...
    def process_batch_for_training(self, batch, B_ot):
        ...
        input_batch = dict()
        input_batch["obs"] = {k: batch["obs"][k][:, :To, :] for k in batch["obs"]}  #TODO: it should be To:
    ...
    def train_on_batch(self, batch, B_ot, ot_params, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            ...
            if ot_params["label"] == 'eef_pose':
                src_label = batch["obs"]["robot0_eef_pos"][:B_ot_src].view(B_ot_src, -1).detach()
                tgt_label = batch["obs"]["robot0_eef_pos"][B_ot_src:B_ot_src+B_ot_tgt].view(B_ot_tgt, -1).detach()
            else:
                src_label = batch["actions"][:B_ot_src, :1].view(B_ot_src, -1).detach()
                tgt_label = batch["actions"][B_ot_src:B_ot_src+B_ot_tgt, :1].view(B_ot_tgt, -1).detach()

            bc_feat = obs_features[B_ot_src+B_ot_tgt:]
            bc_act_label = batch['actions'][B_ot_src+B_ot_tgt:]
    ...
    def log_info(self, info):
        ...
        """
        log = super(OTDiffusionPolicyUNet, self).log_info(info)
