...
def get_ot_dataset(config, obs_keys):
    try:
        no_window = config.algo.ot.no_window
    except:
        no_window = False
    ot_dataset = ChunkSamplingDTWTrainDataset(
        dataset_to_paths=config.train.ot.dataset,
        pair_info_path=config.train.ot.pair_info_path,
        sharpness=config.algo.ot.sharpness,
        cutoff=config.algo.ot.cutoff,
        window_size=config.algo.ot.window_size,
        no_window=no_window,
        dataset_masks=config.train.ot.dataset_masks,
        obs_keys=obs_keys,
...
def train(config, device, auto_remove_exp_dir=False, resume=False):
    """
    ...
    print(config)
    print("")
    log_dir, ckpt_dir, video_dir, vis_dir, time_dir = TrainUtils.get_exp_dir(config, auto_remove_exp_dir=auto_remove_exp_dir, resume=resume)
    ...
    fig_dir = os.path.join(time_dir, 'figures')
    ...
    # load OT training data
    ot_dataset = get_ot_dataset(config, obs_keys=shape_meta["all_obs_keys"])
    ot_loader = DataLoader(
        dataset=ot_dataset,
        batch_size=int(0.5*config.train.ot.batch_size),
        shuffle=True,
        drop_last=True,
    ...
    ot_params=dict(
        emb_scale=config.algo.ot.emb_scale,
        cost_scale=config.algo.ot.cost_scale,
        reg=config.algo.ot.reg,
        tau1=config.algo.ot.tau1,
        tau2=config.algo.ot.tau2,
        scale=config.algo.ot.scale,
        heuristic=config.algo.ot.heuristic,
        label=config.algo.ot.label,
    )
    ...
    for epoch in range(start_epoch, config.train.num_epochs + 1):
        step_log = TrainUtils.run_epoch_for_ot_policy(
            model=model,
            ot_params=ot_params,
            bc_dataloader=train_loader,
            ot_dataloader=ot_loader,
            epoch=epoch,
        ...
        pi = step_log.pop("pi", None)
        if pi is not None:

            # Plot pi using matplotlib
            plt.figure(figsize=(5, 5))
            plt.imshow(pi, cmap='viridis', aspect='auto')
            plt.colorbar(label="Value")
            plt.title("Pi")

            # Save the heatmap to a BytesIO object
            fig_path = os.path.join(fig_dir, "pi_epoch_{}.png".format(epoch))
            plt.savefig(fig_path)
            plt.close()

            # Log the heatmap as an image
            data_logger.record("Train/pi", fig_path, epoch, data_type='image')
        ...
        if config.experiment.rollout.enabled and (epoch > config.experiment.rollout.warmstart) and rollout_check:
            ...
            for k, v in video_paths.items():
                data_logger.record(k, v, epoch, data_type='video', caption=k)
...
def main(args):
    ...
    try:
        train(config, device=device, auto_remove_exp_dir=args.auto_remove_exp, resume=args.resume)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="cuda:0",
        help="(optional) specify the device to run training on (e.g. 'cuda:0' or 'cpu').",
    )
