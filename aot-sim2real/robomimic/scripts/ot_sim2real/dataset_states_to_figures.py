...
def dataset_states_to_obs(args):
    ...
    img_dir = os.path.join(os.path.dirname(args.dataset), f"{os.path.splitext(os.path.basename(args.dataset))[0]}_{args.domain_shift}_imgs")
    ...
    for ind in range(len(demos)):
        ...
        if is_robosuite_env:
            initial_state["model"] = f["data/{}".format(ep)].attrs["model_file"] # TODO: check and fix
        ...
        images = traj["obs"]["agentview_image"]
        for i in range(len(images)):
            img = images[i]
            if img is not None:
                img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                img_path = os.path.join(img_dir, f"{ep}_{i}_agentview_image.png")
                cv2.imwrite(img_path, img)
