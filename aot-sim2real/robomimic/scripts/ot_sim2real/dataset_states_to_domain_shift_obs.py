...
def dataset_states_to_obs(args):
    ...
    EnvUtils.apply_domain_shift(env_meta, domain_shift=args.domain_shift)
    if args.shape_id is not None:
        env_meta["env_kwargs"]["shape_id"] = args.shape_id
    ...
    if args.debug:
        img_dir = os.path.join(os.path.dirname(args.dataset), f"{os.path.splitext(os.path.basename(args.dataset))[0]}_{args.domain_shift}_debug_imgs")
        os.makedirs(img_dir, exist_ok=True)
        obs = env.reset()
        img = obs["agentview_image"]
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        img_path = os.path.join(img_dir, f"debug_img.png")
    ...
    for ind in range(len(demos)):
        ...
        if "data/{}/joint_configs".format(ep) in f.keys():
            joint_configs = f["data/{}/joint_configs".format(ep)][()]
            ep_data_grp.create_dataset("joint_configs", data=np.array(joint_configs))
        ...
        for k in traj["obs"]:
            if k == "agentview_image":
                # save image observations as uint8
                if args.debug:
                    img = cv2.cvtColor(traj["obs"][k][0], cv2.COLOR_RGB2BGR)
                    img_path = os.path.join(img_dir, f"{ep}_{k}.png")
                    cv2.imwrite(img_path, img)
            if args.compress:
    ...
    # save domain_shift info if applicable
    if args.domain_shift is not None:
        data_grp.attrs["domain_shift"] = args.domain_shift
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(optional) domain shift to apply to the environment",
    )

    parser.add_argument(
        "--debug",
        action='store_true',
        help="(optional) debug mode, saves images ",
    )
