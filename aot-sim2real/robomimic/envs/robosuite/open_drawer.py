...
from robomimic.assets.objects import DrawerRL2Object, CoffeeMachinePodObject

from robomimic.envs.robosuite.stack import modify_xml_for_camera_movement
from robosuite.utils.binding_utils import MjRenderContextOffscreen, MjSim


class OpenDrawer_RL2(SingleArmEnv_MG):
    """
    ...
    def _load_model(self):
        ...
        self.drawer_object = DrawerRL2Object(name="DrawerObject")
        self.coffee_pod_object = CoffeeMachinePodObject(name="CoffeePodObject")

        self.objects = [self.drawer_object, self.coffee_pod_object]
        ...
        ranges = self.objs_bound["drawer"]
        x, y, z_rot = ranges["x"], ranges["y"], ranges["z_rot"]

        self.placement_initializer.append_sampler(
            sampler=UniformRandomSampler(
                name=f"DrawerSampler",
                mujoco_objects=self.drawer_object,
                x_range=[x[0], x[1]],
                y_range=[y[0], y[1]],
        ...
        ranges = self.objs_bound["coffee_pod"]
        x, y, z_rot = ranges["x"], ranges["y"], ranges["z_rot"]

        self.placement_initializer.append_sampler(
            sampler=UniformRandomSampler(
                name=f"CoffeePodSampler",
                mujoco_objects=self.coffee_pod_object,
                x_range=[x[0], x[1]],
                y_range=[y[0], y[1]],
                rotation=(z_rot[0], z_rot[1]),
                rotation_axis='z',
    ...
    def _setup_references(self):
        ...
        self.drawer_qpos_addr = self.sim.model.get_joint_qpos_addr(self.drawer_object.joints[0])
        self.drawer_qvel_addr = self.sim.model.get_joint_qvel_addr(self.drawer_object.joints[0])
        self.drawer_body_id = self.sim.model.body_name2id(self.drawer_object.root_body)

        self.pod_body_id = self.sim.model.body_name2id(self.coffee_pod_object.root_body)
    ...
    def _reset_internal(self):
        ...
        self.sim.data.qpos[self.drawer_qpos_addr] = 0.06 + np.random.uniform(-0.01, 0.01)
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def drawer_quat(obs_cache):
                return T.convert_quat(np.array(self.sim.data.body_xquat[self.drawer_body_id]), to="xyzw")

            sensors = [drawer_pos, drawer_quat]
    ...
    def _get_initial_placement_bounds(self):
        ...
        """
        obj_bounds = {}
        obj_bounds["drawer"] = dict(
                x=(0.15, 0.17),
                y=(-0.11, -0.09),
                z_rot=(-0.01 * np.pi, + 0.01 * np.pi),
                # NOTE: hardcoded @self.table_offset since this might be called in init function
                reference=np.array((0, 0, 0.912)),
            )
        obj_bounds["coffee_pod"] = dict(
            x=(-0.03, 0.03),
            y=(0.1, 0.16),
            z_rot=(0-0.05 * np.pi, 0 + 0.05 * np.pi),
            # NOTE: hardcoded @self.table_offset since this might be called in init function
            reference=np.array((0, 0, 0.912)),
        )
        return obj_bounds
    ...
    def _check_drawer_close(self):
        ...
        return drawer_qpos < 0.03

    def _in_drawer(self, obj_pos):

        drawer_pos = np.array(self.sim.data.body_xpos[self.drawer_body_id])
        drawer_rot_mat = T.quat2mat(T.convert_quat(np.array(self.sim.data.body_xquat[self.drawer_body_id]), to="xyzw"))
        res = False

        error_in_drawer_frame = drawer_rot_mat.T @ (obj_pos - drawer_pos)

        drawer_qpos = self.sim.data.qpos[self.drawer_qpos_addr]

        if (
            abs(error_in_drawer_frame[0] - drawer_qpos) < 0.08
            and abs(error_in_drawer_frame[1]) < 0.08
            and obj_pos[2] < self.table_offset[2] + 0.04
        ):
            res = True

        # print("x:", error_in_drawer_frame[0] - drawer_qpos, "y:", error_in_drawer_frame[1], "z:", obj_pos[2] - self.table_offset[2])
    ...
    def _check_success(self):
        ...
        drawer_close = self._check_drawer_close()
        in_drawer = self._in_drawer(pod_pos)
        # print("drawer close:", drawer_close)
        # print("in drawer:", in_drawer)

        result = drawer_close and in_drawer

        return result
    
class OpenDrawer_RL2_Range(OpenDrawer_RL2):
