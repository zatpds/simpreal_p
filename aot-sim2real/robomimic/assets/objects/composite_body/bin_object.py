...
class ElevatedBinObject(CompositeObject):
    """
    Generates a four-walled bin container with an open top.
    Args:
        name (str): Name of this Bin object
    ...
    def _get_geom_attrs(self):
        ...
        # Initialize dict of obj args that we'll pass to the CompositeObject constructor
        base_args = {
            "total_size": (self.bin_size + np.array([0, 0, self.elevation])) / 2.0,
            "name": self.name,
            "locations_relative_to_center": True,
            "obj_types": "all",
            "density": self.density,
        ...
        add_to_dict(
            dic=obj_args,
            geom_types="box",
            geom_locations=(0, 0, -(self.bin_size[2] - self.wall_thickness) / 2 - self.elevation/2),
            geom_quats=(1, 0, 0, 0),
            geom_sizes=np.array((self.bin_size[0], self.bin_size[1], self.wall_thickness+self.elevation))/2,
            geom_names=self._base_geom,
            geom_rgbas=None if self.use_texture else self.rgba,
            geom_materials=self.bin_mat_name if self.use_texture else None,
            geom_frictions=self.friction,
        ...
        add_to_dict(
            dic=obj_args,
            geom_types="box",
            geom_locations=(-(self.bin_size[2] - self.wall_thickness) / 2, 0, 0),
            geom_quats=(1, 0, 0, 0),
            geom_sizes=np.array((self.wall_thickness, self.bin_size[1], self.bin_size[2]-self.wall_thickness*2))/2,
            geom_names=f"back",
            geom_rgbas=wall_rgba,
            geom_materials=wall_mat,
            geom_frictions=self.friction,
        ...
        add_to_dict(
            dic=obj_args,
            geom_types="box",
            geom_locations=(0, -(self.bin_size[1] - self.wall_thickness) / 2, 0),
            geom_quats=(1, 0, 0, 0),
            geom_sizes=np.array((self.bin_size[0], self.wall_thickness, self.bin_size[2]-self.wall_thickness*2))/2,
            geom_names=f"left",
            geom_rgbas=wall_rgba,
            geom_materials=wall_mat,
            geom_frictions=self.friction,
        ...
        add_to_dict(
            ...
            geom_locations=(0, 0, +(self.bin_size[2] - self.wall_thickness) / 2),
            geom_quats=(1, 0, 0, 0),
            geom_sizes=np.array((self.bin_size[0], self.bin_size[1], self.wall_thickness))/2,
            geom_names=f"top",
            geom_rgbas=wall_rgba,
            geom_materials=wall_mat,
            geom_frictions=self.friction,
