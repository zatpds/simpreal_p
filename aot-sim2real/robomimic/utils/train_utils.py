...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    ...
    ds_weights = []
    for i in range(len(config.train.data)):
        ds_cfg = config.train.data[i]
        if ds_cfg.get("demo_limit", config.train.demo_limit) > 0:
            ds_kwargs["hdf5_path"].append(ds_cfg["path"])
            ds_kwargs["filter_by_attribute"].append(ds_cfg.get("filter_key", filter_by_attribute))
            ds_kwargs["demo_limit"].append(ds_cfg.get("demo_limit", config.train.demo_limit))
            ds_weights.append(ds_cfg.get("weight", 1.0))
        else:
            continue
...
def run_rollout(
    ...
    is_env_wrapper = isinstance(env, EnvWrapper)
    ...
    for step_i in range(horizon): #LogUtils.tqdm(range(horizon)):
        ...
        # process abs action
        if "joint_configs" in policy.policy.global_config.train.action_keys:
            target_jpos = ac[:7]
            if is_env_wrapper:
                delta_jpos = target_jpos - env.env.env.robots[0]._joint_positions
                norm_jpos = delta_jpos / env.env.env.robots[0].controller.output_max
            else:
                delta_jpos = target_jpos - env.env.robots[0]._joint_positions
                norm_jpos = delta_jpos / env.env.robots[0].controller.output_max

            norm_action = np.concatenate([norm_jpos, [ac[-1]]])
...
def run_epoch_for_ot_policy(model, ot_params, bc_dataloader, ot_dataloader, epoch, 
                            validate=False, num_steps=None, obs_normalization_stats=None):
    """
    Run an epoch of training or validation.
    ...
    ot_dataloader = infinite_dataloader(ot_dataloader)
    bc_dataloader = infinite_dataloader(bc_dataloader)

    for _ in LogUtils.custom_tqdm(range(num_steps)):

        # load next batch from data loader
        t = time.time()
        ot_batch = next(ot_dataloader)
        bc_batch = next(bc_dataloader)
        ...
        #  batch, B_ot_src, B_ot_tgt, ot_params, epoch, validate=False

        # forward and backward pass
        t = time.time()
        info = model.train_on_batch(input_batch, B_ot, ot_params, epoch, validate=validate)
