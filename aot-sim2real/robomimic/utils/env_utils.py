...
def apply_domain_shift(env_meta, domain_shift=None):
    if domain_shift is not None:
        env_meta["env_kwargs"]["domain_shift"] = domain_shift
        if domain_shift == "cam_pose":
            shifted_euler = [-2.519, 0.0, 1.9]
            shifted_position = [0.87, 0.13, 0.42]
            mat3x3 = t3d.euler.euler2mat(shifted_euler[0], shifted_euler[1], shifted_euler[2], axes='sxyz')
            mat4x4 = t3d.affines.compose(shifted_position, mat3x3, [1, 1, 1])
            env_meta["env_kwargs"]["cam_pose"] = mat4x4.tolist()
        elif domain_shift == "teaser_view":
            shifted_euler = [-2.35, 0.0, 1.57]
            shifted_position = [0.92, 0.0, 0.54]
            mat3x3 = t3d.euler.euler2mat(shifted_euler[0], shifted_euler[1], shifted_euler[2], axes='sxyz')
            mat4x4 = t3d.affines.compose(shifted_position, mat3x3, [1, 1, 1])
            env_meta["env_kwargs"]["cam_pose"] = mat4x4.tolist()       
            env_meta["env_kwargs"]["cam_K"] = [
                [426.67, 0.0, 320.0],
                [0.0, 480.0, 240.0],
                [0.0, 0.0, 1.0]
            ] 
