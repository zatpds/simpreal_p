...
class DataLogger(object):
    ...
    def __init__(self, log_dir, config, log_tb=True, log_wandb=False):
        ...
        if log_wandb:
            ...
            # try resuming from previous run if it exists
            # Path to the latest run ID
            try:
                pattern = os.path.join(log_dir, "wandb", "latest-run", "run-*.wandb")
                run_files = glob.glob(pattern)

                # Extract ID from filename
                latest_file = os.path.basename(run_files[0])
                match = re.match(r"run-(\w+)\.wandb", latest_file)
                run_id = match.group(1)
                print("Resuming from previous run ID: {}".format(run_id))
            except:
                run_id = None
    ...
    def record(self, k, v, epoch, data_type='scalar', log_stats=False, caption=None):
        """
        ...
        assert data_type in ['scalar', 'image', 'video'], "data_type must be one of ['scalar', 'image', 'video']"
        ...
        if self._wandb_logger is not None:
            try:
                if data_type == 'scalar':
                    ...
                    self._wandb_logger.log({k: wandb.Video(v, caption, fps=20)}, step=epoch)
            except Exception as e:
