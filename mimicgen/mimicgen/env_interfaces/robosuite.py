...
class RobosuiteInterface(MG_EnvInterface):
    ...
    def _get_arm_controller(self):
        """
        Helper to get the arm controller, compatible with both robosuite v1.4.x and v1.5.x.
        """
        ...
        if hasattr(robot, 'controller'):
            ...
            # robosuite >= 1.5.x uses composite_controller
            return robot.composite_controller.part_controllers['right']

    def _get_eef_name(self):
        """
        Helper to get the end effector site name, compatible with both robosuite v1.4.x and v1.5.x.
        """
        robot = self.env.robots[0]
        if hasattr(robot, 'controller') and hasattr(robot.controller, 'eef_name'):
            # robosuite <= 1.4.x
            return robot.controller.eef_name
        else:
            # robosuite >= 1.5.x
            return robot.gripper['right'].important_sites['grip_site']
    ...
    def target_pose_to_action(self, target_pose, relative=True):
        ...
        max_drot = arm_controller.output_max[3]
